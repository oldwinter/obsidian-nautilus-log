...
pure module decides typed execution operations from immutable task, CLOCK,
settings, time, and generated-identity facts; it performs no I/O and knows no
source text, Obsidian object, mutation queue, or host adapter. Ticket #26 retains
all existing primary requirement ownership and gains no host or Markdown write
authority.

The reviewed execution-application seam assigns the new
`src/runtime/execution/application.ts` module to ticket #27. The module exposes
one small surface-facing interface: subscribe to immutable confirmed snapshots
and dispatch a typed user intent. Its implementation owns the queue-head fresh
read, call to the pure #26 decision module, `MutationPlan` and
`MutationExpectation` preparation, and authoritative outcome publication needed
to connect ticket #26's runtime to ticket #27's host adapters. The injected
`TextAccess`, identity index, runtime projection, paired clock, ID generator, and
committer remain internal seams; surface adapters never receive source text,
source spans, mutation plans, or expectations. This handoff changes no primary
requirement owner: ticket #26 retains CLOCK/POMO state-machine and command-
decision authority and ticket #25 retains the only Markdown write implementation.
...
| `NautilusRuntime` | One vault-scoped lifecycle, settings/POMO saves, one mutation queue, refresh generations, snapshots, active-CLOCK projection, fresh intent preparation, cancellation/reconciliation | Markdown syntax, concrete views, a durable projection or command journal |
