...
malformed Plan Region is already present. A malformed-region notice names
today's Daily Note and **Locate Primary Plan** after Execution Layer is on.
If the region is already there and
...
a repair. It does not say only that source navigation is disabled. **Show
details** on that sidebar uses the same localized next steps. It does not
expose a raw code such as `focused-task-unavailable`. A degraded state means the
