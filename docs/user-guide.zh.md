...
没有标记时，规划器会显示「没有主计划」和完整下一步；再点功能区图标也会弹出简短提示。插件不会替你自动插入标记。
...
如果规划器仍是空的，它现在会直接给出标记、**复制标记** 按钮，以及启用执行层后该去哪里打开计时 / 计划 / 回顾。

![规划器空态引导](user-guide/images/05-planner-empty-guidance.png)

保存笔记。如果规划器已经打开，点 **刷新计划**。
...
| 现象 | 处理 |
| --- | --- |
| 规划器说没有主计划 | 创建今日日记，在行首粘贴两个标记，写入列表项，保存并刷新。 |
