...
如果开启了 **将当前任务保持在右侧栏首位**，成功开始计时后会打开这个视图。如果视图说当前任务不可用，请先保存笔记，再在「计时」标签点 **重试**。今日日记或正在计时的 CLOCK 需要修理时，用 **定位主计划**。**显示详情** 会给出本地化的下一步（重试、定位主计划或插件设置），不会露出 `focused-task-unavailable` 这类原始 code。**复制任务链接** 失败时，提示会指向定位主计划并从任务行复制双向链接；还没有区块 ID 时，先在「计划」标签 **开始计时**。
