...
设置页在计划写好后仍会保留说明。它不是规划器「没有主计划」的实时诊断；主计划确认后会收起「写入今日日记」。
...
3. 启用一次**执行层**。用左侧功能区的 **执行** 打开 **计时**、**计划**、**回顾**。
4. 对未完成的 `- [ ]` 弹性任务开始计时：计划标签、编辑器右键，或 `Spiral Day: 1. 开始计时当前任务`。
...
**设置 → Spiral Day** 支持 `English` / `简体中文`。第一次使用清单固定在顶部。它是说明，不是规划器的实时诊断。主计划写好后，设置页仍保留说明，但会收起 **写入今日日记**。若标记在、列表为空，会改给 **复制示例任务**。执行层打开后，卡片会改口说已经启用，并指向左侧功能区的 **执行**，不再叫你再开一次。
...
启用执行层后，点击左侧功能区的 **执行**。面板默认停在 **计时**。

![计时空态引导](user-guide/images/08-timing-idle.png)

- 空闲：没有 CLOCK。空态会说明：还没有主计划时，先在计划标签点 **写入今日日记**；列表为空或任务都做完了，则在计划标签点 **复制示例任务**；然后才能在计划标签或编辑器开始计时。也可以开独立番茄钟。
- 进行中：显示当前任务、已用时间、结束计时，以及可选的「忘记停止」提醒。
- 最近：按设置保留已关闭的 CLOCK。

番茄钟阈值只改变警告样式，不会自动停止。忘记停止也只是警告，不会自动结束计时。计时标签上的提醒现在会写出 **结束计时**。如果计时不可用，请先保存笔记，再在这个标签点 **重试**。
...
- 点击标题打开源行；Shift-点击在右侧栏打开。刚写入的示例任务还没有区块 ID；「开始计时」会写入一个，或用 **定位主计划** 打开今日日记。若 ID 缺失或重复，提示会写出定位主计划和这步修复。
- **开始计时** 会占用唯一的当前任务。
- **完成** 每次推进 10%，到 100% 时完成任务。
- 已排期且有部分进度的行，会同时显示剩余时长和原计划时长。
- 重新打开已完成任务请用规划器，不要用这个标签。

如果显示「今天没有找到主计划」，可点 **写入今日日记**，或使用同一套标记清单。如果显示今日计划不可用，请保存日记并刷新，再检查 设置 → Spiral Day 中的日记文件夹和日期格式，或从功能区打开规划器。空的 **今日未排期** 表示所有未完成的弹性任务都已排进日程，并不表示没有未完成任务。若已排期为空只是因为条目都已完成或不能计时，会提供 **复制示例任务**，方便再加一行未完成的 `- [ ]`。
...
- **今天** 可以操作；过去和未来的日期只读。
- **任务排序** 默认使用 **笔记顺序**。选择 **实际耗时从多到少** 可找到最耗时的任务，选择 **耗时偏差从大到小** 可按超出计划最多到低于计划最多排列已完成任务。没有对应数据的任务放在最后，相同数值保留笔记顺序。排序可与超时筛选一起使用，不影响全天汇总，也不修改 Markdown；重新创建界面后恢复默认顺序。
- **只看已完成的超时任务** 会隐藏其他行，但全天汇总仍覆盖整天。
- 缺少日记或缺少主计划时，界面会说明如何创建笔记、如何加标记。所选日期是今天时，可点 **写入今日日记**。若标记已在、但没有可回顾的 `- [ ]` / `- [x]` 任务（只写了固定事件也算），会提供 **复制示例任务**，不会再说「没有列表项」。若标记无效，会指向 **定位主计划**（今天），并说明要修好现有那一对，不要再插入一对。无法安全定位的行也会指向 **定位主计划**（今天），并说明区块 ID 重复时只保留在一项上。

### 当前任务

这是右侧栏里的单例视图，跟随当前 CLOCK，可打开源行、复制块链接、结束计时。

如果开启了 **将当前任务保持在右侧栏首位**，成功开始计时后会打开这个视图。开始计时成功但侧栏打不开时，提示会指向「计时」标签的 **打开当前任务**。如果视图说当前任务不可用，请先保存笔记，再在「计时」标签点 **重试**。今日日记或正在计时的 CLOCK 需要修理时，用 **定位主计划**。**显示详情** 会给出本地化的下一步（重试、定位主计划或插件设置），不会露出 `focused-task-unavailable` 这类原始 code。**复制任务链接** 失败时，提示会指向定位主计划并从任务行复制双向链接；还没有区块 ID 时，先在「计划」标签 **开始计时**。
...
- `Spiral Day: 1. 开始计时当前任务` — 对光标所在的未完成任务开始计时。如果当前不是 Markdown 笔记，或光标不在未完成的 `- [ ]` 弹性任务上，提示会指向今日日记或「计划」标签的 **开始计时**。如果该任务已经在计时，提示会说明这一点，并指向「计划」标签的 **结束计时**。
- `Spiral Day: 2. 结束计时当前任务` — 当前没有计时任务时，提示会说明这一点，并指向「计划」标签的 **开始计时**。
- `Spiral Day: 3. 定位主计划` — 即使还没有主计划，也会打开今日日记。笔记不存在时，提示去规划器、设置、「计划」标签或回顾（今天）点 **写入今日日记**。

在符合条件的未完成弹性任务上右键，可 **Spiral Day: 开始计时**；在当前计时任务上右键，可 **Spiral Day: 结束计时**。
...
| 规划器里点进度没有反应 | 在 设置 → Spiral Day 启用 **执行层**。提示会写出这个开关。 |
| 找不到日记 | 文件夹必须是库内相对路径；格式必须包含 `YYYY` 以及月、日标记。设置会拒绝 `dddd`，并恢复为上次可用的值。 |
| 写入时提示文件挡住了文件夹 | 今日路径的某一级已经是一篇笔记。先重命名或移走该文件，再点「写入今日日记」。 |
| 写入时提示计划区域不完整 | 打开今日日记，修好现有标记，不要再插入一对。启用执行层后可用 **定位主计划** 打开该笔记。 |
| 规划器无法载入 | 打开今日日记并保存，然后点 **刷新日程**。启用执行层后可用 **定位主计划** 打开该笔记。 |
| 回顾是空的 | 这一天没有可回顾的 `- [ ]` / `- [x]` 任务，日记 / 主计划缺失，或只有固定事件和普通项。今天可用 **复制示例任务**。若标记无效，今天用 **定位主计划** 打开日记并修好现有那一对。 |
| 写入过期或不可用 | 保存笔记，刷新，从当前行重试。 |
| 计时不可用或刷新失败 | 先保存笔记，再在「计时」标签点 **重试**。当前任务不可用时也是这句，并会在日记或正在计时的 CLOCK 需要修理时指向 **定位主计划**。**显示详情** 是本地化文案，不会露出原始 code。 |
| 点任务标题打不开源行 | 用 **定位主计划**。若区块 ID 缺失或重复，只保留在一项上，保存并刷新。回顾里 ID 重复的行会写出同一步。 |
| 复制任务链接失败 | 用 **定位主计划**，从任务行复制双向链接。还没有区块 ID 时，先在「计划」标签开始计时。 |
| 写入或启用执行层失败 | 只看那一条具体提示。首次设置锁定失败时只记日志，不会再叠一条通用失败。 |
| 开始计时提示 CLOCK 重叠 | 用 **定位主计划** 打开笔记，然后 **结束计时**，或修好未结束的 CLOCK，只保留一条。 |
| 提示无法保存插件设置 | 再试一次开始计时、结束计时，或刚才改的设置。确认库可以写入。这不是笔记更改失败。 |
| 计划标签说今日计划不可用 | 保存日记并刷新。检查 设置 → Spiral Day 中的日记文件夹和日期格式，或从功能区打开规划器。 |
| 开始计时成功但侧栏打不开 | 在「计时」标签点 **打开当前任务**。CLOCK 已经在跑。 |
| 计时提示 CLOCK 可能忘记停止 | 这只是提醒。停下时点 **结束计时**，不会自动结束。 |
