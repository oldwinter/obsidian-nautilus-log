...
没有标记时，规划器会显示「没有主计划」和完整下一步；再点功能区图标也会弹出简短提示。打开规划器不会自动插入标记。需要插件写入时，在规划器、设置或回顾（今天）点 **写入今日日记**：它会按需创建今日笔记，并写入标记加一条示例任务。

![设置页的第一次使用清单](user-guide/images/02-settings-first-run.png)

### 第一次使用清单

1. 打开 **设置 → Spiral Day**。第一次会跟着 Obsidian 语言走；若核心日记插件已有配置，会复制它的文件夹和日期格式。否则保持默认，或改成和你现有日记一致。
2. 点 **写入今日日记**。没有今日笔记时会按需创建（默认是库根目录下的 `YYYY-MM-DD.md`），并写入标记加一条示例任务。也可以自己建笔记再粘贴两个标记。
...
| 设置 | 默认 | 为什么重要 |
| --- | --- | --- |
| 语言 | 第一次跟着 Obsidian（`en` / `zh`） | 立刻切换规划器、执行界面、命令面板、编辑器菜单和规划器功能区文案。保存过之后以插件设置为准。 |
| 日记文件夹 | 第一次复制核心日记插件，否则为空（库根目录） | 必须和「写入今日日记」创建、规划器读取的笔记一致。 |
| 日记日期格式 | 第一次复制核心日记插件，否则 `YYYY-MM-DD` | 必须包含年、月、日标记。 |
...
如果显示「今天没有找到主计划」，可点 **写入今日日记**，或使用同一套标记清单。
