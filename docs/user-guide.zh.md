...
如果开启了 **将当前任务保持在右侧栏首位**，成功开始计时后会打开这个视图。如果视图说当前任务不可用，请先保存笔记，再在「计时」标签点 **重试**。今日日记或正在计时的 CLOCK 需要修理时，用 **定位主计划**。**复制任务链接** 失败时，提示会指向定位主计划并从任务行复制双向链接；还没有区块 ID 时，先在「计划」标签 **开始计时**。
...
| 计时不可用或刷新失败 | 先保存笔记，再在「计时」标签点 **重试**。当前任务不可用时也是这句，并会在日记或正在计时的 CLOCK 需要修理时指向 **定位主计划**。 |
