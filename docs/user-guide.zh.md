...
如果规划器仍是空的，它现在会直接给出标记加示例任务、**复制计划模板**、**写入今日日记**，以及启用执行层后该去哪里打开计时 / 计划 / 回顾。
...
| 日记文件夹 | 第一次启用时从核心日记插件复制，否则为空（库根目录） | 必须和「写入今日日记」创建、规划器读取的笔记一致。写入或改设置后会锁定，之后改核心日记插件不会挪路径。 |
| 日记日期格式 | 第一次启用时从核心日记插件复制，否则 `YYYY-MM-DD` | 必须包含年、月、日标记。写入或改设置后会锁定。 |
