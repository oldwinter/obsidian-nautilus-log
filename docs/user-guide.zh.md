...
- 缺少日记或缺少主计划时，界面会说明如何创建笔记、如何加标记。所选日期是今天时，可点 **写入今日日记**。若标记已在、但没有可回顾的 `- [ ]` / `- [x]` 任务（只写了固定事件也算），会提供 **复制示例任务**，不会再说「没有列表项」。
...
| 回顾是空的 | 这一天没有可回顾的 `- [ ]` / `- [x]` 任务，日记 / 主计划缺失，或只有固定事件和普通项。今天可用 **复制示例任务**。 |
