...
- 缺少日记或缺少主计划时，界面会说明如何创建笔记、如何加标记。所选日期是今天时，可点 **写入今日日记**。若标记已在、但没有可回顾的 `- [ ]` / `- [x]` 任务（只写了固定事件也算），会提供 **复制示例任务**，不会再说「没有列表项」。若标记无效，会指向 **定位主计划**（今天），并说明要修好现有那一对，不要再插入一对。
...
| 点任务标题打不开源行 | 用 **定位主计划**。若区块 ID 缺失或重复，只保留在一项上，保存并刷新。 |
