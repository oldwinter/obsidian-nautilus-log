...
启用后，左侧功能区会出现贝壳图标 **Open Spiral Day**。这是规划器。计时、计划、回顾和命令面板动作此时都还没有注册。
...
没有标记时，规划器会显示「没有主计划」和完整下一步；再点功能区图标也会弹出简短提示。打开规划器不会自动插入标记。需要插件写入时，点 **写入今日日记**：它会按需创建今日笔记，并写入标记加一条示例任务。

![设置页的第一次使用清单](user-guide/images/02-settings-first-run.png)

### 第一次使用清单

1. 打开 **设置 → Spiral Day**。第一次建议保持默认，或把 **日记文件夹**、**日记日期格式** 改成和你现有日记一致。
2. 创建今日日记。默认是库根目录下的 `YYYY-MM-DD.md`。
3. 点 **写入今日日记**，或自行粘贴两个标记并在中间写入直接列表项。
4. 点击功能区的 **Open Spiral Day**。
5. 需要 CLOCK、番茄钟、计时、计划、回顾、当前任务和命令时，再启用 **执行层**。
...
如果规划器仍是空的，它现在会直接给出标记、**复制标记**、**写入今日日记**，以及启用执行层后该去哪里打开计时 / 计划 / 回顾。
...
4. 对未完成的 `- [ ]` 弹性任务开始计时：计划标签、编辑器右键，或 `Spiral Day: 1. Focus current block`。
...
**设置 → Spiral Day** 支持 `English` / `简体中文`。第一次使用清单固定在顶部。

入门时最重要的几项：

| 设置 | 默认 | 为什么重要 |
| --- | --- | --- |
| 语言 | English | 立刻切换规划器和执行界面文案。 |
| 日记文件夹 | 空（库根目录） | 必须和你正在编辑的日记所在目录一致。 |
| 日记日期格式 | `YYYY-MM-DD` | 必须包含年、月、日标记。 |
...
通过功能区贝壳图标 **Open Spiral Day** 打开，始终对应今天的配置日记。
...
- 空闲：没有 CLOCK。空态会告诉你去计划标签或编辑器开始计时。也可以开独立番茄钟。
...
- `Spiral Day: 1. Focus current block` — 对光标所在的未完成任务开始计时
- `Spiral Day: 2. Clock out Timing Line`
- `Spiral Day: 3. Locate Primary Plan`

在符合条件的未完成弹性任务上右键，可 **开始计时**；在当前计时任务上右键，可 **结束计时**。
...
| 现象 | 处理 |
| --- | --- |
| 规划器说没有主计划 | 点 **写入今日日记**，或在行首粘贴两个标记，写入列表项，保存并刷新。 |
| 装完只有一个功能区图标 | 正常。执行层打开后才有其他界面。 |
| 某一行没有「开始计时」 | 必须是直接的未完成 `- [ ]` 弹性任务，不能是固定事件或嵌套项。 |
| 命令面板找不到命令 | 先启用执行层，再搜索 `Spiral Day:`。 |
| 找不到日记 | 文件夹必须是库内相对路径；格式必须包含 `YYYY` 以及月、日标记。 |
| 回顾是空的 | 这一天没有可回顾任务，或日记 / 主计划缺失。 |
