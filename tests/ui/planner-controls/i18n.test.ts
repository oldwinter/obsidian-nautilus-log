...
test("host chrome and empty-state copy stay bilingual after Review insert", () => {
  ...
  assert.match(enReview["state.invalidPlan"], /Locate Primary Plan/);
  assert.match(enReview["state.invalidPlan"], /Repair the existing markers/);
  assert.equal(enReview["state.invalidPlan"].includes("Check the Daily Note"), false);
  ...
  assert.match(enExecution["active.unavailableDetail"], /Try again on the Timing tab/);
  assert.match(enExecution["active.unavailableDetail"], /Locate Primary Plan/);
  assert.equal(enExecution["active.unavailableDetail"].includes("Source navigation"), false);
  ...
  assert.match(enExecution["plan.unavailableDetail"], /Settings → Spiral Day/);
  assert.match(enExecution["plan.unavailableDetail"], /open Planner from the ribbon/);
  assert.match(zhCNExecution["plan.unavailableDetail"], /设置 → Spiral Day/);
  assert.match(zhCNExecution["plan.unavailableDetail"], /功能区打开规划器/);
  assert.match(enExecution["timing.forgotten"], /warning only/);
