...
try {
  ...
  await scenario("visibility-and-destroy", async () => {
    ...
    await page.getByText("No reviewable `- [ ]` or `- [x]` tasks today. Use Copy sample task and paste an open `- [ ]` flexible task between the markers, then save and refresh.", { exact: true }).waitFor();
